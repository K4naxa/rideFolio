...
<template>
  ...
  >
    ...
    <div class="hidden lg:flex items-center gap-2 ml-auto pr-4 lg:pr-6 text-sm">
      <Button
        variant="outline"
        @click="modalStore.onOpen('createRefill')"
        class="border-refill hover:bg-refill/10 text-refill hover:shadow-sm hover:shadow-refill/50"
      >
        <Icons.refill class="text-refill" size="sm" />
        <span class="hidden 2xl:block">Refill</span>
      </Button>
      <Button variant="outline" class="border-maintenance hover:bg-maintenance/10 text-maintenance">
