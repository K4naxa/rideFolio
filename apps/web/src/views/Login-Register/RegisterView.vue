...
type RegisterFormValues = z.infer<typeof refinedSchema>;

const authError = ref<string | null>(null);

const form = useForm<RegisterFormValues>({
  validationSchema: refinedSchema,
  initialValues: {
    username: "",
    email: "",
    password: "",
    ...
    },
    {
      onError: ({ error }) => {
        if (error.status === 422 && error.code === "USER_ALREADY_EXISTS_USE_ANOTHER_EMAIL") {
          form.setFieldError("email", "User already exists. Use another email.");
          return;
        }
          ...
          <form @submit.prevent="onSubmit" novalidate :aria-busy="form.isSubmitting.value" data-cy="register-form">
            <CardContent class="flex flex-col gap-6 px-6">
              ...
              <div class="flex w-full items-center gap-2">
                ...
                <span class="text-muted-foreground relative z-10 shrink-0 px-2"> Or register with </span>
                <div class="bg-muted h-px w-full rounded-full" />
              </div>

              <div class="grid grid-cols-3 gap-4">
                <Button variant="outline" type="button" disabled aria-label="Register with Apple">
                  ...
                  <span class="sr-only">Register with Apple</span>
                </Button>
                <Button variant="outline" type="button" disabled aria-label="Register with Google">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                    <path
                ...
                <Button variant="outline" type="button" disabled aria-label="Register with Meta">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                    <path
